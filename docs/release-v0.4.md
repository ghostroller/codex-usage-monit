...
`v0.4.0` 已于 2026-09-08 发布，标签提交为 `afd9f021`。
以下内容保留发布前的审查快照；后续 CI 修复与当前节省重复验证的操作规则见
[测试工作流](testing.md)。后续发版请使用该工作流，不重复执行本文件的历史步骤。
...
3. 当时的发布流程：核对 Linux、macOS、Windows 和依赖审计结果。只有成功后才创建并推送
