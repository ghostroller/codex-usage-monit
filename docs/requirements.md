...
- 同时显示 exact local token share、observed quota delta、estimated quota、evidence coverage、confidence、unattributed、cycle proxy 和 external activity risk；
- 所有任务结束后可以把 `settled` 标为 true并把严格观测估算置信度提高到 Medium，但包含 cycle proxy 的估算保持 Low，任何结果都不能标为 exact quota；
...
- 默认使用 Flat 列表；`R` 与可点击 `[R]Tree` 在 Flat/Tree 间切换。Tree 仅把拥有可靠直接父 thread id 的 subagent 挂到当前过滤结果中可见的父节点下，支持多层关系；父节点缺失或被过滤时 child 作为根节点，损坏的自引用/循环关系不得卡死渲染。拥有子节点的行显示稳定宽度的 `[-]` / `[+]` 按钮；Tasks 聚焦时 `-` 收起、`+` 展开所选父节点，整块按钮可点击且不得误选相邻行。Tree 模式提供大写 `E` 与可点击 `[E]Collapse`，一次收起当前文本和来源条件形成的完整过滤树中所有父节点，包含已隐藏在折叠祖先下的嵌套父节点。折叠状态按 thread id 跨刷新保留，选择不得停留在被折叠隐藏的后代。节点折叠时，父行的 token breakdown 与所选 reset cycle local share 必须汇总当前过滤树内所有隐藏后代；同一额度桶内 estimated quota 可汇总并采用最保守置信度，跨桶分支必须显示未知而不能相加百分点。展开后恢复逐行值，不能改写 `Snapshot` 或在父子同时可见时重复计数。挂在可见父节点下的 child 省略与父节点重复的项目名，orphan root 仍显示项目名。树根与同层分支按包含隐藏后代在内的子树最新活动项排序，让新活动 child 带动整组到顶部；
...
同一 duration 的账户百分比不得跨桶混合。对已知的 `codex` / `codex_bengalfox` 桶，明确模型必须按固定规则归桶：仅 `gpt-5.3-codex-spark` 使用 `codex_bengalfox`，其他非空模型使用 `codex`；两个桶存在时各按自己的 reset 边界与观测生成独立分析，只存在一个时也不得把映射到缺失桶的调用回退到现有桶，scope 必须标为 partial 并警告。缺失/空模型不得猜测归桶，与其重叠的额度增量保持 unattributed。task/turn 仅落入一个桶时显示该桶 estimated quota；跨桶时显示未知，不得相加两个额度池的百分点。其他未知多桶组合继续使用保守选择：优先 Codex 产品桶和 `ServerSnapshot` provenance，再按稳定排序选择一个边界并输出 warning；未选候选仍显示 gauge，并禁用 task/turn/model 的桶级 estimated quota。
...
工具同时构建当前 5 小时和周 reset cycle 的窗口分析；模型映射时同一 duration 可以包含多个桶分析。每个 `windowAnalyses` 项必须独立携带 `partial` 和 `partialReasons`，不能用 Week 的不完整状态污染完整的 5h，也不能用一个桶的数据质量污染另一个桶。周周期的本地 token share 在扫描完整覆盖各桶周期起点且没有截断、坏行、不可读文件、未映射调用或歧义 counter reset 时是本地可观察范围内的精确值；额度百分点归因仍然只能标为 estimated。`--days` 小于覆盖所选 reset cycle 所需的范围时，只将对应窗口分析标为 partial，即使已扫描到的文件本身都可读。
...
- 未覆盖部分保留为 `unattributedPercent`，并显式标记外部活动仍可能存在；
- 对扫描完整、当前、非 stale 且模型归桶完整的周期，将证据缺口按该额度桶的本地 token share 投影到 task/turn/model，记为 `proxyProjectedPercent`；行级 estimated quota 是严格证据贡献与该代理贡献之和，并以 `~` 和 Low confidence 显示；
- `proxyProjectedPercent` 是 `unattributedPercent` 中被投影的部分，两者重叠而非相加；`estimatedAssignedPercent + unattributedPercent = usedPercent` 保持证据不变量，evidence coverage 不因代理提高；
- quota 百分比同周期回调只重启严格证据 epoch；只要当前服务端窗口和本地周期扫描仍完整，Low cycle proxy 可以继续生成并保留 correction warning；
- 扫描/回看不完整、stale、未知模型、映射目标桶缺失或未知多桶时禁用 cycle proxy；
...
- quota gauges；
- Overview 最顶栏中的 `[M]Models`、`[5h]` / `[Week]` 按钮；`M`、`5` / `W` 是真实快捷键，整块按钮支持鼠标左键点击，并遵循 btop 风格快捷键字符强调规则；
- 所选当前 reset cycle 的 duration、实际起止时间和额度；
- 使用所选 scope 的 task 表；
- 选中 task 的 turns，包含消息摘要、可点击选中态和 turn 详情；
- Models 面板先显示所选 scope 的 observed evidence、完整 estimated、proxy、unverified gap、evidence coverage/confidence 归因摘要，再显示按 token 降序的模型表；已知模型映射产生多个桶时逐桶显示摘要，并用各桶内已映射 token 的合并分母重算模型 local share；空间不足时标记 `top N/M`，并区分“scope 不可用”“窗口内没有本地模型调用”“已知多桶映射”和“未知多个活跃额度桶导致估算禁用”；
...
- Overview 最顶栏中的 `5` / `W` 和鼠标按钮会同步切换 Tasks、Turns、Models 与归因摘要，`M` 和 `[M]Models` 可稳定隐藏并恢复 Models；
- Tree 节点收起时父行准确汇总当前过滤树中隐藏后代的 token、local share 与 estimated quota，展开时不重复计数；
- Fast turn 的 `FAST` 只出现在模型名称后，普通 turn 的模型单元格保持不变；
- 相同 duration 已知 `codex` / `codex_bengalfox` 桶无论出现一个或两个都按模型隔离归因，缺失模型与缺失目标桶 partial，跨桶实体不合并额度百分点；离线 fallback 同时保留各桶最新观测；其他多桶选择 Codex/Server 优先、输出 warning，未选桶保持 gauge-only；
