...
TUI 将主题、顶层视图、window scope、API 长上下文倍率开关、Turns/Models 显隐、全局 table-column 开关、Flat/Tree 和来源筛选保存为版本化的用户级 JSON。读取失败或内容损坏时回退到默认值；开关字段缺失时回退安全默认值。搜索、选择、滚动位置和具体 thread 折叠集合不持久化。写入采用同目录临时文件替换，未来版本文件不会被旧程序覆盖；`--theme` 显式值优先于保存值。CLI 一次性输出保持基础口径，不参与此状态生命周期。
