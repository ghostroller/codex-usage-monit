...
4. `fast` 与本地登录态 rollout 中兼容的 `priority` service tier 都作为 ChatGPT Fast；按官方 [Speed](https://learn.chatgpt.com/docs/agent-configuration/speed) 对 GPT-5.6/GPT-5.5 family 应用 `2.5x`，对 GPT-5.4 family 应用 `2x`，不支持 Fast 的已知模型保持 Standard；该兼容行为不等同于官方另行计费的 API Priority。当前 Codex credit 卡没有 cache-write 行，因此公式不增加 cache-write credits；
...
9. token-based 映射使用 estimator revision 3；只从仍在配置扫描范围内的 rollout 调用重建重叠本地桶/周数据点，并在新点的未加权 token/call 证据不差于旧点时通过 revision-aware upsert 替换；无法重建的 revision 1/2 历史点保留原 revision 并隔离，混合 revision 不得合并，EST 保持 unavailable/partial。
...
- attribution：5h/Week reset-cycle 边界、排除滚动 `now-duration` 口径、reset drift、只选择 `codex`、含 `gpt-5.6` Sol 别名和当前 Daybreak aliases/IDs 的完整 token-based credit 费率矩阵、`fast`/`priority` 与 Standard、Spark 精确模型名大小写不敏感排除、缺失模型的 Luna 后备、task/turn/model 公式求和、estimator revision 3、扫描范围内 rollout 重建与 revision-aware upsert、revision 1/2 保留及混合 revision 隔离、partial/stale 保留 Low estimate、无分母 unavailable，以及 `codex_bengalfox` gauge-only；
