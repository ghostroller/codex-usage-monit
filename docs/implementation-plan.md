...
新增独立 `ui_state` 模块，把主题、View、WindowScope、默认 false 的 `apiLongContextMultiplier`、Turns/Models 显隐、TaskListMode 和 TaskSourceFilter 映射为版本化 camelCase JSON。路径按 `CODEX_USAGE_MONIT_STATE_DIR`、`XDG_STATE_HOME`、平台用户级 state 目录依次解析；损坏/缺失字段回退默认值，未来 schema 禁止旧版本覆盖，写入使用私有目录和同目录原子替换。事件处理前后只比较上述稳定偏好，有变化时写回并在正常退出兜底；搜索、焦点、selection、viewport、临时 Turns 与 thread 折叠集合留在进程内。
