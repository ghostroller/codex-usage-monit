...
当前部署来源策略：Settings 的 **B Deploy agent** 与 `remote deploy HOST` 通过 SSH 让远端直接从固定官方 GitHub 仓库下载对应版本的 Release agent；中心不选择或上传本地可执行文件。清单由 release 打包流程自动生成，远端在执行候选程序前校验源码 build ID、版本、协议、目标平台、文件大小和 SHA-256，再安装到隔离目录并验证原 source identity 和数据可用性，最后以配置 revision 检查原子切换。缺少发布包、开发源码与发布构建不同、校验或探测失败，都保留原配置，绝不自动回退旧协议或本地上传。`remote deploy-dev HOST --bundle-dir DIR` 是显式开发专用入口，运行前提示信任边界，TUI 不提供此路径。构建 ID 和摘要不是发布者签名；正式路径依赖官方仓库、发布流程和 HTTPS 的可信性，当前未验证独立签名或 artifact attestation。远端无需源码或编译环境；平台依赖和错误分类详见 [部署说明](remote-usage.md#inspect-and-deploy-a-matching-agent)。

当前版本管理策略：项目处于快速迭代阶段，**不补旧数据协议兼容，不协商降级，也不通过丢弃 quota 数据继续使用旧协议**。中心与 exporter 的数据协议、数据 revision 和模型目录必须匹配，单看 `0.4.0` 版本号不足以判断兼容性。新增独立于数据帧协议的 `remote-agent info`，输出版本、源码构建标识和目标平台；`remote inspect HOST` 用于明确诊断，Settings 的 **B Deploy agent** / `remote deploy HOST` 安装匹配构建。安装采用版本与摘要隔离的私有目录，校验制品和原 source identity，通过数据 probe 后才原子切换本地配置；不覆盖全局程序或重启远端 recorder。旧 agent 的安装引导不等于兼容其数据协议。开发版跨平台制品准备和发布制品规则见 [版本与部署策略](remote-usage.md#version-policy-during-rapid-iteration)。
