...
当前部署来源策略：Settings 的 **B Update node** 与 `remote deploy HOST` 通过 SSH 让远端直接从固定官方 GitHub 仓库下载对应版本的 Release agent；中心不选择或上传本地可执行文件。清单由 release 打包流程自动生成，远端在执行候选程序前校验源码 build ID、版本、协议、目标平台、文件大小和 SHA-256，再安装到隔离目录并验证原 source identity 和数据可用性，最后以配置 revision 检查原子切换。缺少发布包、开发源码与发布构建不同、校验或探测失败，都保留原配置，绝不自动回退旧协议或本地上传。`remote deploy-dev HOST --bundle-dir DIR` 是显式开发专用入口，运行前提示信任边界，TUI 不提供此路径。构建 ID 和摘要不是发布者签名；正式路径依赖官方仓库、发布流程和 HTTPS 的可信性，当前未验证独立签名或 artifact attestation。远端无需源码或编译环境；平台依赖和错误分类详见 [部署说明](remote-usage.md#inspect-and-deploy-a-matching-agent)。

当前版本管理策略：项目处于快速迭代阶段，**不补旧数据协议兼容，不协商降级，也不通过丢弃 quota 数据继续使用旧协议**。中心与 exporter 的数据协议、数据 revision 和模型目录必须匹配，单看 `0.4.0` 版本号不足以判断兼容性。新增独立于数据帧协议的 `remote-agent info`，输出版本、源码构建标识和目标平台；`remote inspect HOST` 用于明确诊断，Settings 的 **B Update node** / `remote deploy HOST` 安装匹配构建。安装采用版本与摘要隔离的私有目录，校验制品和原 source identity，通过数据 probe 后才原子切换本地配置；不覆盖全局程序；同时按已注册的参数和启用状态替换远端 recorder，并验证新构建的历史落盘心跳。已配对且启用的源在激活后执行有界真实同步；升级事务、失败恢复和缓存代次规则见 [远端更新流程](remote-updates.md)。旧 agent 的安装引导不等于兼容其数据协议。开发版跨平台制品准备和发布制品规则见 [版本与部署策略](remote-usage.md#version-policy-during-rapid-iteration)。

当前 quota 扩展：remote protocol v5 同步各来源本机 recorder 保存的额度观测，按 source/generation 独立持久化，显式确认同账号后才投影合并为 GLOBAL account 历史；原始本机账户记录和 reset credits 不被远端改写。详见 [额度历史同步与合并规则](remote-usage.md#synchronize-quota-remaining-history)。

- 状态：0.4.0 release candidate，本地验证完成，待集中 CI
- 目标版本：0.4.0
- 更新日期：2026-09-08

用户操作与升级步骤见 [SSH 用量汇总指南](remote-usage.md)。2026-09-08 的发布前复核已完成实际 macOS/Linux SSH 配对、回填、增量、副本 facts、逐来源对账与离线保留验证，以及 Windows 中心到 macOS exporter 的真实 SSH 配对和同步；完整平台检查和发布状态记录在 [0.4 发布准备](release-v0.4.md)。本设计中的后续研究项不代表已承诺的稳定能力。
...
Overview 的 5 小时/周 task 与 turn 用量另由中心本地的 `AllIncluded` 统一历史查询投影，不能直接累加各 source bucket，也不能受 Summary/Trends 当前 source selector 影响。统一查询先完成 logical-replica authority/union 去重；投影随后把远端物理 ID 规范化为带 NodeId 的 namespace，补齐选定窗口内的历史行和祖先闭包，再用 live 层覆盖近期状态与元数据。窗口只计入起点不早于服务端窗口且采样内容没有越过窗口结束时间的本地桶；无法安全切分的边缘桶或缺失覆盖保持 lower-bound/partial。analysis miss 必须显示零/未知，绝不回退到累计 live counter。Models 使用同一份去重后的 `AllIncluded` history，按模型/服务层形成全来源统一总额；它不要求、也绝不从 aggregate model bucket 推断 task、session 或 turn 的逐项模型拆分。旧远端历史若未持久化逐模型 API cost，Models 的 API EQ 必须保留为明确的 lower bound，直到回放或 bootstrap 导入更丰富的数据。Summary 数值同样只来自 source-aware history bucket，live 行只可作为零用量元数据，不能重复计数。
...
- 原子文件替换与隔离重命名使用支持长路径的 `MOVEFILE_WRITE_THROUGH`；若原子文件替换因旧文件仍被身份/CAS reader 持有而返回 access denied，使用 Rust 的 POSIX rename 兼容路径，旧 reader 继续读取原文件，并对新文件的同一写句柄在替换前后 flush。该兼容路径没有 write-through rename 标志，文件 flush 不承诺父目录项在断电后仍然持久；隔离重命名仍坚持 write-through 且禁止覆盖已有 trash。普通目录只以只读 `BACKUP_SEMANTICS | OPEN_REPARSE_POINT` 句柄验证稳定身份，不申请目录写权限，也不调用文档未保证支持的目录 `FlushFileBuffers`；
