...
def fixtures(target="aarch64-apple-darwin"):
    expected = {"schemaVersion": 1, "product": "codex-usage-monit", "version": "0.4.0",
                "buildId": "1" * 64, "target": target, "protocolVersion": 5}
    filename = f"codex-usage-monit-{target}.agent" + (".exe" if "windows" in target else "")
    return expected, {"schemaVersion": 1, "agent": copy.deepcopy(expected), "file": filename,
                      "size": len(BODY), "sha256": hashlib.sha256(BODY).hexdigest()}


class UnixReleaseBootstrapTests(unittest.TestCase):
    def exercise(self, mutation=None, binary=BODY):
    ...
    def test_prepares_matching_release_without_executing_it(self):
        ...
        self.assertEqual(calls, [base + "codex-usage-monit-aarch64-apple-darwin.agent.json",
                                 base + "codex-usage-monit-aarch64-apple-darwin.agent"])

    def test_rejects_metadata_mismatch_before_downloading_binary(self):
        for field, value in (("buildId", "2" * 64), ("target", "x86_64-apple-darwin"),
                             ("version", "0.3.0"), ("protocolVersion", 4), ("schemaVersion", True)):
            with self.subTest(field=field):
                error, calls = self.exercise(lambda m: m["agent"].update({field: value}))
...
class WindowsReleaseBootstrapTests(unittest.TestCase):
    def test_both_shells_prepare_and_reject_invalid_release_without_execution(self):
        ...
        for shell in shells:
            for case in ("success", "missing", "wrong_build", "wrong_protocol", "bad_path", "tampered", "oversize"):
                with self.subTest(shell=shell, case=case), tempfile.TemporaryDirectory(prefix="release preparation ") as root:
                    ...
                    if case == "wrong_build":
                        manifest["agent"]["buildId"] = "2" * 64
                    elif case == "wrong_protocol":
                        manifest["agent"]["protocolVersion"] = 4
                    elif case == "bad_path":
                        manifest["file"] = "../other.exe"
                    elif case == "oversize":
                        manifest["size"] = 134217729
...
function Receive-ReleaseAsset {{
    ...
    if (-not $Url.StartsWith('https://github.com/ghostroller/codex-usage-monit/releases/download/v0.4.0/')) {{ throw 'wrong origin' }}
    $global:Downloads++
...
"""
                    ...
                    if case == "success":
                        ...
                        self.assertEqual(result["hash"], manifest["sha256"])
