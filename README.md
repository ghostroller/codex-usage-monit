...
The estimator follows OpenAI's current [token-based Codex rate card](https://learn.chatgpt.com/docs/pricing). GPT-6 Astra uses `(250, 25, 1250)` input/cached-input/output credits per 1M tokens. `gpt-5.6` (the Sol alias), `gpt-5.6-sol`, and Daybreak Blue's current `daybreak-blue-latest` / `gpt-daybreak-blue-latest` aliases use `(100, 10, 500)`; OpenAI documents this Sol promotional pricing as available at least through November 21, 2026. Daybreak Red's current `daybreak-red-latest` / `gpt-daybreak-red-latest` aliases and `gpt-5.6-cyber` ID use `(312.5, 31.25, 1875)`, while the legacy `gpt-5.5-cyber` slug remains mapped to that row for historical rollouts. The current card also supplies the GPT-5.6 Terra/Luna, GPT-5.5, GPT-5.4, and GPT-5.4 mini rows. GPT-5.3-Codex, GPT-5.2, and the historical `gpt-5.2-codex` slug retain earlier compatibility weights but are not presented as current official card rows.

For recognized ChatGPT Fast calls the estimator applies the published [Speed](https://learn.chatgpt.com/docs/agent-configuration/speed) multiplier: `2.5x` for GPT-6 Astra, GPT-5.6, and GPT-5.5, and `2x` for GPT-5.4. The compatible `serviceTier=priority` value found in local signed-in rollouts is treated as Fast for this attribution; this is not API Priority billing, which the official Speed page describes separately. Exact `gpt-5.3-codex-spark` calls remain outside this attribution because its credit rate is still a research preview; an unlisted or missing non-Spark model uses the active catalog's explicit credit fallback and marks the scope partial.
