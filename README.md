...
- **API equivalent is token-only and observation-backed.** It values model calls observed locally or imported from explicitly included remote sources at the bundled current API prices. The Models panel is a unified all-source model/service-tier aggregate after replica deduplication; it does not infer a model split for an individual task, session, or turn. It excludes per-call tool fees, container, storage, search-call, tax, regional, and negotiated-contract charges; an unsynchronized source remains absent rather than being estimated. Remote history written before per-model API cost persistence remains an explicit lower bound until it is replayed or bootstrapped with richer data.
