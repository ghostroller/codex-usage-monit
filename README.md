...
v0.4 can use a central machine to pull normalized aggregate history, bounded session facts, and a compact Overview snapshot from another machine over the system OpenSSH client. The remote runs the same executable as a short-lived exporter; it does not need a listening port, database, or resident remote service, and raw rollout JSONL is not copied to the center. Current account quota gauges and reset credits still come only from the central machine. Recorded quota remaining history also synchronizes, and Settings → Remote sources → **O Same account quota** explicitly opts a source into the merged account history. Keep this off for other accounts; the default is off.
...
The **Overview** tab combines account limits with Tasks, Turns, Models, and the token-only API-equivalent cost of model calls observed across all included sources. Its weekly quota gauge also shows an expiry reminder when a fully known available Codex reset credit expires before the current server-defined weekly reset. **Trends** shows remaining quota, weekly local token and estimate trajectories, and 15-minute bars. **Summary** ranks project usage for the current cycle, 7 days, or 30 days and provides a collapsed project/session/user-turn tree, top-project bars, and a non-cumulative stacked-area chart colored by project. A turn row includes that root turn's own model usage plus all descendants of a subagent branch that was exactly spawned by that turn; unavailable legacy links remain in a compact `Unassigned delegated usage` row instead of being guessed from timestamps. The wide layout keeps the tree and ranking side by side above a full-width chart. Cycle the chart through local `1d`, `12h`, `6h`, `3h`, and `1h` buckets; it defaults to the top six projects plus `Other`, or can show all projects. Summary preserves the selected range, metric, chart grain, and project-set preference across restarts. Click or drag across an uncompressed plot to inspect an exact time bucket, or use Inspect with keyboard navigation. Exact inspection is disabled when a long fine-grained series must be compressed to fit the terminal. Each project's stable color is reused for its tree swatch, ranking bar, legend marker, and chart area. The chart labels complete (`C`), partial (`P`), and missing (`M`) buckets; missing project-level history is left as a gap instead of being plotted as zero. **Other** shows source health, collection statistics, diagnostics, quota windows, reset-credit details, and recorder health. **Settings** provides one place to manage display preferences and the metric columns shared by all tables.
...
The estimator follows OpenAI's current [token-based Codex rate card](https://learn.chatgpt.com/docs/pricing). The exact IDs `gpt-6-astra`, `gpt-6-sol`, and `gpt-6-luna` use `(250, 25, 1250)`, `(50, 5, 250)`, and `(2.5, 0.25, 12.5)` input/cached-input/output credits per 1M tokens respectively, checked against the official card on September 26, 2026. `gpt-5.6` (the GPT-5.6 Sol alias), `gpt-5.6-sol`, and Daybreak Blue's current `daybreak-blue-latest` / `gpt-daybreak-blue-latest` aliases use `(100, 10, 500)`; OpenAI documents this GPT-5.6 Sol promotional pricing as available at least through November 21, 2026. Daybreak Red's current `daybreak-red-latest` / `gpt-daybreak-red-latest` aliases and `gpt-5.6-cyber` ID use `(312.5, 31.25, 1875)`, while the legacy `gpt-5.5-cyber` slug remains mapped to that row for historical rollouts. The current card also supplies the GPT-5.6 Terra/Luna, GPT-5.5, GPT-5.4, and GPT-5.4 mini rows. GPT-5.3-Codex, GPT-5.2, and the historical `gpt-5.2-codex` slug retain earlier compatibility weights but are not presented as current official card rows.

For recognized ChatGPT Fast calls the estimator applies the published [Speed](https://learn.chatgpt.com/docs/agent-configuration/speed) multiplier: `2.5x` for GPT-6 Astra/Sol/Luna, GPT-5.6, and GPT-5.5, and `2x` for GPT-5.4. The compatible `serviceTier=priority` value found in local signed-in rollouts is treated as Fast for this attribution; this is not API Priority billing, which the official Speed page describes separately. Exact `gpt-5.3-codex-spark` calls remain outside this attribution because its credit rate is still a research preview; an unlisted or missing non-Spark model uses the active catalog's explicit credit fallback and marks the scope partial.
...
At process startup, the monitor looks for `model-catalog.json` in its normal configuration directory: `~/Library/Application Support/codex-usage-monit` on macOS, `$XDG_CONFIG_HOME/codex-usage-monit` or `~/.config/codex-usage-monit` on Linux, and `%LOCALAPPDATA%\codex-usage-monit` on Windows. `CODEX_USAGE_MONIT_CONFIG_DIR` overrides that directory consistently with the other configuration files. The file is optional and is never created automatically: when it is absent, the bundled catalog is used; when it exists but cannot be read or validated, commands that calculate pricing or negotiate pricing revisions fail instead of silently using different rates.

Start from [the complete example catalog](docs/model-catalog.example.json). An external file is a complete replacement, including aliases, Codex Standard/Fast credit rates, the unknown-model credit fallback, API Standard/Fast short/long rates, cache-write support, metadata, and the long-context threshold. Decimal strings are accepted so rates such as `31.25` remain exact. Its `estimatorRevision` and `apiPricingCatalogRevision` must initially be greater than the bundled revisions (currently 7 and 4), and the affected revision must increase whenever its mappings or rates change. Restart the TUI and recorder after editing; no rebuild is required. Synchronized machines must use the same revisions and the same normalized catalog fingerprint; remote protocol v5 rejects any mismatch before merging data.
...
- **API equivalent is token-only and observation-backed.** It values model calls observed locally or imported from explicitly included remote sources at the active API catalog prices. The Models panel is a unified all-source model/service-tier aggregate after replica deduplication; it does not infer a model split for an individual task, session, or turn. It excludes per-call tool fees, container, storage, search-call, tax, regional, and negotiated-contract charges; an unsynchronized source remains absent rather than being estimated. Remote history written before per-model API cost persistence remains an explicit lower bound until it is replayed or bootstrapped with richer data. If an already-ingested remote generation carries a different catalog fingerprint, its raw token/call evidence remains visible but its old EST/API-derived values are masked and marked partial until a compatible resync.
...
| `apiPricing` | Version, rates-as-of date, source URL, and the `current_api_rates_model_tokens_only` basis of the active bundled or configured API price catalog. |
...
- [Windows and UTM testing](docs/windows-testing.md)
