# Windows user installer. Run with Windows PowerShell 5.1 or PowerShell 7.
# This file embeds the shared release verifier so offline installation needs no
# source checkout/network access. tests/test_windows_install.py prevents drift.
[CmdletBinding()]
param(
    [string]$Version = 'latest',
    [string]$Bundle,
    [switch]$NoModifyPath,
    [ValidateSet('none', 'on-logon')][string]$Recorder = 'none',
    [switch]$Adopt,
    [switch]$AllowDevBuild
)

# BEGIN SHARED RELEASE VERIFIER (src/remote_agent_manager/release_bootstrap.ps1)
...
function Invoke-VerifiedInstaller {
    ...
    # Keep native stderr diagnostic output from becoming a terminating error in
    # Windows PowerShell; the executable's actual exit code decides success.
    $ErrorActionPreference = 'Continue'
    $global:LASTEXITCODE = 0
    & $Candidate @Arguments
    $invocationSucceeded = $?
    $code = $LASTEXITCODE
    if (-not $invocationSucceeded -and $code -eq 0) { $code = 1 }
    if ($code -ne 0) {
        $nativeFailure = New-Object InvalidOperationException("install_candidate_failed: native exit code $code")
        $nativeFailure.Data['NativeExitCode'] = [int]$code
        throw $nativeFailure
    }
}

function Invoke-UserInstallation {
    param([string]$RequestedVersion, [string]$OfflineBundle, [bool]$SkipPath,
        [string]$RecorderMode, [bool]$AdoptEntry, [bool]$PermitDevBuild)
    $ErrorActionPreference = 'Stop'
    $normalizedVersion = $RequestedVersion -creplace '^v', ''
    if ($normalizedVersion -cnotmatch '^(latest|[0-9]+\.[0-9]+\.[0-9]+(?:[-+][A-Za-z0-9.+-]+)?)$') {
        throw 'install_version_invalid: expected latest or [v]X.Y.Z'
    }
    if ($env:OS -ne 'Windows_NT') { throw 'install_platform_unsupported: Windows is required' }
    $architecture = if ($env:PROCESSOR_ARCHITEW6432) { $env:PROCESSOR_ARCHITEW6432 } else { $env:PROCESSOR_ARCHITECTURE }
    if ($architecture -notin @('AMD64', 'ARM64')) {
        throw 'install_platform_unsupported: release requires Windows x64 (or x64 emulation on ARM64)'
    }
    if ($OfflineBundle) {
        $script:InstallerBundle = (Resolve-Path -LiteralPath $OfflineBundle).ProviderPath
        if (-not (Test-Path -LiteralPath $script:InstallerBundle -PathType Container)) {
            throw 'install_bundle_invalid: expected directory containing release-manifest.json and release executable'
        }
        # Acquisition is the only offline difference. The exact shared manifest,
        # byte-size, target, filename and SHA256 checks still run below.
        function Receive-ReleaseAsset {
            param([string]$Url, [string]$Destination, [long]$Maximum)
            $name = ($Url -split '/')[-1]
            if ($name -cnotin @('release-manifest.json', 'codex-usage-monit-x86_64-pc-windows-msvc.exe')) {
                throw 'install_bundle_invalid: unexpected requested asset'
            }
            $source = Join-Path $script:InstallerBundle $name
            $item = Get-Item -LiteralPath $source
            if ($item.PSIsContainer -or ($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -or $item.Length -gt $Maximum) {
                throw 'install_bundle_invalid: expected bounded ordinary file'
            }
            Copy-Item -LiteralPath $source -Destination $Destination
        }
    }
    $temporaryRoot = [IO.Path]::GetFullPath([IO.Path]::GetTempPath())
    $stage = '.codex-usage-monit-release-' + [Guid]::NewGuid().ToString('N')
    $directory = Join-Path $temporaryRoot $stage
    # Reject any path escape before cleanup. Only our exact random stage can be
    # removed; the bundle and installation directory are never cleanup targets.
    $resolvedStage = [IO.Path]::GetFullPath($directory)
    if ([IO.Path]::GetDirectoryName($resolvedStage).TrimEnd('\') -cne $temporaryRoot.TrimEnd('\')) {
        throw 'install_staging_invalid: stage escaped the temporary directory'
    }
    Push-Location -LiteralPath $temporaryRoot
    try {
        $expected = [pscustomobject]@{ target = 'x86_64-pc-windows-msvc'; version = $normalizedVersion }
        $prepared = Invoke-ReleasePreparation $expected $stage
        $candidate = Join-Path $directory 'agent.exe'
        $manifest = Join-Path $directory 'manifest.json'
        # Candidate execution begins only after shared byte verification. Rust
        # then checks its embedded build/protocol identity against that manifest.
        Invoke-VerifiedInstaller $candidate @('update', 'verify-release', '--manifest', $manifest,
            '--target', $expected.target, '--version', $prepared.agent.version)
        $arguments = @('install', '--current-binary', '--version', $prepared.agent.version,
            '--recorder', $RecorderMode)
        if ($SkipPath) { $arguments += '--no-modify-path' }
        if ($AdoptEntry) { $arguments += '--adopt' }
        if ($PermitDevBuild) { $arguments += '--allow-dev-build' }
        Invoke-VerifiedInstaller $candidate $arguments
    } finally {
        Pop-Location
        # The shared preparation routine owns a private, unique staging folder.
        # Enumerate known files only: no recursive removal or following links.
        foreach ($name in @('agent.exe', 'manifest.json')) {
            $file = Join-Path $resolvedStage $name
            if (Test-Path -LiteralPath $file) { Remove-Item -LiteralPath $file -Force -ErrorAction SilentlyContinue }
        }
        if (Test-Path -LiteralPath $resolvedStage) {
            try { [IO.Directory]::Delete($resolvedStage, $false) } catch {}
        }
    }
}

# Dot-sourcing exposes the bounded functions for isolated fixture tests only.
# A normal -File invocation always installs and propagates failures as exit 1.
if ($MyInvocation.InvocationName -ne '.') {
    try {
        Invoke-UserInstallation $Version $Bundle ([bool]$NoModifyPath) $Recorder ([bool]$Adopt) ([bool]$AllowDevBuild)
        exit 0
    } catch {
