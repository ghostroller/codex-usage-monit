...
**Overview** tab 把账户额度、Tasks、Turns、Models，以及所有已 included 来源中观察到的模型调用的纯 token API 等价费用放在同一页面。如果信息完整的可用 Codex 重置机会会在当前服务端周自然重置前过期，提醒会直接显示在周用量进度条内。**Trends** 显示剩余额度、本地周 token/估算走势和 15 分钟柱状图。**Summary** 按本周期、近 7 天或近 30 天排行项目，并提供默认收起的项目/session/用户对话轮树、项目柱状图，以及按项目着色的非累计堆叠面积图。每个对话轮显示根会话本轮自身的模型用量，加上能够确认由本轮创建的整个 subagent 分支；缺少精确关联的旧数据会集中显示为 `Unassigned delegated usage`，不会按时间猜测。宽屏布局把树和项目排行以 1:1 放在上方，图表在下方占满宽度；图表可在本地时间 `1d`、`12h`、`6h`、`3h`、`1h` 桶之间循环。图表默认显示 Top 6 + Other，也可切换到全部项目；所选时间范围、指标、图表粒度和项目集合会在重启后恢复。在未压缩的图内单击或拖动可查看准确时间桶，也可打开 Inspect 后用键盘切换；当长区间的细粒度点数超过终端绘图区时，精确 Inspect 会禁用，避免把压缩列冒充某个小时。每个项目的稳定颜色会复用于树中的色块、排行柱、图例标记和图表面积。`C`、`P`、`M` 分别表示完整、部分和缺失时间桶；缺少项目级历史的桶会留空，而不是绘制成零。**Other** 显示数据源健康状态、采集统计、诊断信息、额度窗口、重置机会详情和后台 recorder 状态。**Settings** 集中管理显示偏好和所有表格共用的指标列。
...
| Inspect 模式中的 `←` / `→`、`Home`、`End` | 选择上一个/下一个、第一个或最后一个已记录数据点。 |
| Inspect 模式中的 `↑` / `↓` | 在当前可见图表之间移动。 |
| `5`、`w` | 选择 5 小时或周重置周期。 |
...
| `API EQ.` / `API.EQ5H` | 按当前生效的 OpenAI API 费率目录换算本地模型 token。TUI 中每个 Tasks 和 Turns 行都显示当前所选 5 小时或周 scope 内的独立值；它不是 lifetime 总额，窗口缺失时显示 `-`，且不随 `EST Longx` 切换。一次性 task/turn 行固定使用当前 5 小时重置周期。区间表示请求边界可能是短或长上下文；末尾 `+` 表示这是下界，因为未计价 sample 或不完整的本地 rollout 覆盖可能遗漏额外费用；`-` 也表示没有可用本地数据或没有可应用的公开价格。 |
...
- **API 等价费用只覆盖有观察证据的模型 token。** 它按当前生效目录的 API 价格换算本机观察到、或从显式 included 远程来源导入的模型调用。Models 面板会在 replica 去重后，按模型/服务层汇总全部来源；不会为单个 task、session 或 turn 猜测模型拆分。它不包含按次工具费、容器、存储、搜索调用、税费、区域加价或协商合同价格；未同步来源会保持缺失，不会被估算补齐。旧版远端历史尚未持久化逐模型 API 费用时，会明确保持为下限，直至通过回放或 bootstrap 补充更完整的数据。已摄取远端 generation 的目录指纹若不同，原始 token/call 证据仍显示，但旧目录生成的 EST/API 派生值会被屏蔽并标为 partial，直到完成兼容目录的重新同步。
...
| --- | --- |
| `asOf` | 快照时间。 |
| `apiPricing` | 当前生效的内置或外部 API 价格目录版本、费率日期、来源 URL，以及 `current_api_rates_model_tokens_only` 计算口径。 |
